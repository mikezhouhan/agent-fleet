...
| Cursor Projects 恢复集 | 项目主管和成员 | 客户端材料不足以证明通用 ACP | 项目、队列、成果与介入界面 | 项目协作产品参考 | 无完整云端源码，非可直接运行底座 |
...
前十一项证据与边界沿用[十一项目综合报告](eleven-project-comparison.zh-CN.md)及其专题；Warp 每项事实在 [Warp 专题](warp-factories-analysis.zh-CN.md)中指向固定源码或官方文档。本表是结构比较，不是性能排名。
...
| 人工批准 | 流程指令和仓库权限；无人值守 CLI 绕过原生权限提示 | 与交互式工具审批分开统计 |
| 本地接手 | MCP pickup 不领取锁，也不暂停云端 | 对照任务 claim、lease 与 fencing，验证重复工作风险 |
...
先确定是否必须拥有完整控制面，以及首版是长期伙伴还是团队交付。若坚持原来的长期伙伴目标，仍应从 Memoh、Omnigent、Cindy 中收敛核心，再借鉴 Kandev 的执行控制、Agent Swarm/Multica 的任务组织和 Warp 的配置评估闭环。不要把这些项目的完整控制面一起拼接。
