...
- [九项目综合比较与更新后的路线建议](analysis/extensions/nine-project-comparison.zh-CN.md)
...
- [下载八个源码仓库与获取 Cursor 恢复材料](SOURCE_DOWNLOAD.zh-CN.md)
- [六个项目的区别、优缺点与架构分析](analysis/multi-agent-grok-product-analysis.zh-CN.md)
- [七条底座演进路线](analysis/routes/README.md)
...
比较对象为 Kandev、Cindy、Grok Bot 0.18 重建版、Rakazo、Agent Orchestrator、DeepSeek Harness、Cursor Projects 客户端恢复材料、Omnigent 与 Agent Swarm。原六项目报告保留其固定基准；2026-09-15 增补报告先后纳入三个新对象。报告基于固定源码快照，源码引用链接锁定到具体提交；完整版本记录见 [sources.json](sources.json)。

当前产出为静态源码分析与演进方案，尚未实现新产品，也未通过实际模型调用验证各项目的性能、稳定性或兼容性。报告区分已有能力、架构推断和拟议改造，不代表项目最新版本的持续评测。

本仓库包含研究报告和 `cursor-projects-reversed/` 客户端恢复证据；八个独立上游仓库需另行下载，其源码、素材与许可证请访问各原始仓库。Grok Bot 研究对象为非官方重建版本，其结论受可见源码范围限制。
